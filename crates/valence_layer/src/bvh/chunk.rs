use std::{mem, ops::Range, marker::PhantomData};
...
impl Aabb<ChunkPos> for ChunkAabb {
...
pub type ChunkBvh<T, const MAX_SURFACE_AREA: i32 = { 8 * 4 }> = Bvh<T, ChunkPos, ChunkAabb, MAX_SURFACE_AREA>;
...
impl<T: GetChunkPos, const MAX_SURFACE_AREA: i32> ChunkBvh<T, MAX_SURFACE_AREA> {
  ...
  fn build_rec(&mut self, bounds: ChunkAabb, value_range: Range<usize>) {
  ...
  pub fn query(&self, view: ChunkView, mut f: impl FnMut(&T)) {
      if let Some(root) = self.nodes.last() {
          ...
          self.query_rec(root, view, ChunkAabb { min, max }, &mut f);
      }
  }

  fn query_rec(&self, node: &Node<ChunkPos, ChunkAabb>, view: ChunkView, view_aabb: ChunkAabb, f: &mut impl FnMut(&T)) {
      match node {
          Node::Internal {
              bounds,
              left,
  ...
  #[cfg(test)]
  fn check_invariants_rec(&self, node: &Node<ChunkPos, ChunkAabb>) {
      match node {
          Node::Internal {
              bounds,
              left,
...
fn value_bounds<T: GetChunkPos>(values: &[T]) -> Option<ChunkAabb> {
  values
      .iter()
      .map(|v| ChunkAabb::point(v.chunk_pos()))
      .reduce(ChunkAabb::union)
}
